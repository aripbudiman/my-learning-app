...
import FormLesson from '../templates/FormLesson.vue'
import QuickActions from '../components/QuickActions.vue'
import ActivityItem from '../components/ActivityItem.vue'
import CourseWithBatches from '../components/CourseWithBatches.vue'
import CourseWithLessons from '../components/CourseWithLessons.vue'
...
const showFormLesson = ref(false)
const formLessonRef = ref(null)
...
const getCoursesWithLessons = async () => {
    try {
        const response = await axios.get(
            `${url}/api/courses/with-lessons?limit=${limit.value.lesson}&page=${page.value.lesson}`
        )
        coursesWithLessons.value = response.data.data
    } catch (error) {
        console.error('Error fetching courses with lessons:', error)
        createToast('Error fetching courses with lessons', {
            type: 'danger',
...
const handleShowFormLesson = () => {
    showFormLesson.value = true
}
...
const handleFormLessonClose = () => {
    showFormLesson.value = false
    formLessonRef.value?.resetForm()
}
...
const handleAddLesson = (batch) => {
    console.log('Add lesson to batch:', batch)
    // TODO: Open lesson form
}

const handleEditLesson = (lesson) => {
    console.log('Edit lesson:', lesson)
    // TODO: Open lesson form with data
}

const handleDeleteLesson = async (lesson) => {
    if (!confirm(`Are you sure you want to delete lesson "${lesson.lessonTitle}"?`)) return

    try {
        await axios.delete(`${url}/api/lessons/${lesson.id}`)
        createToast('Lesson deleted successfully', {
            type: 'success',
            hideProgressBar: 'false',
            showIcon: 'true',
        })
        ...
        createToast('Error deleting lesson', {
            type: 'danger',
            hideProgressBar: 'false',
            showIcon: 'true',
        })
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div role="tablist" aria-orientation="horizontal"
            ...
            <button type="button" role="tab" aria-selected="true"
                :data-state="tab === 'overview' ? 'active' : 'inactive'" class="tab-button" @click="tab = 'overview'">
                <Icon icon="lucide:chart-column" width="17" height="17" class="text-gray-500" />
                <span>Overview</span>
            </button>
            <button type="button" role="tab" aria-selected="false"
                :data-state="tab === 'courses' ? 'active' : 'inactive'" class="tab-button" @click="tab = 'courses'">
                <Icon icon="lucide:book-open" width="17" height="17" class="text-gray-500" />
                <span>Courses</span>
            </button>
            <button type="button" role="tab" aria-selected="false"
                :data-state="tab === 'batches' ? 'active' : 'inactive'" class="tab-button" @click="tab = 'batches'">
                <Icon icon="fluent:layer-24-regular" width="23" height="23" class="text-gray-500" />
                <span>Batches</span>
            </button>
            <button type="button" role="tab" aria-selected="false"
                :data-state="tab === 'lessons' ? 'active' : 'inactive'" class="tab-button" @click="tab = 'lessons'">
        ...
        <div id="tab-courses" v-if="tab === 'courses'">
            <div class="flex items-center justify-between">
                ...
                <button @click="handleShowFormCourse"
            ...
            <FormCourse v-if="showFormCourse" ref="formCourseRef" @submit="handleFormCourseSubmit"
                @update="handleFormCourseUpdate" @cancel="handleFormCourseCancel" @close="handleFormCourseClose" />
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
                <CourseCard v-for="course in courses" :key="course.id" :course="course" @edit="handleEditCourse"
        ...
        <div id="tab-batches" v-if="tab === 'batches'">
            <div class="flex items-center justify-between">
                ...
                <button @click="handleShowFormBatch"
                    class="justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 bg-primary text-primary-foreground shadow hover:bg-primary/90 h-9 px-4 py-2 flex items-center space-x-2">
                    <Icon icon="lucide:plus" width="24" height="24" class="w-4 h-4" />
                    <span>Add New Batch</span>
                </button>
            </div>
            <FormBatch v-if="showFormBatch" ref="formBatchRef" :courses="courses" @submit="handleFormBatchSubmit"
                @update="handleFormBatchUpdate" @cancel="handleFormBatchCancel" @close="handleFormBatchClose" />
            <div class="space-y-8 mt-8">
                <CourseWithBatches v-for="batch in batches" :key="batch.id" :course="batch"
                    @edit-batch="handleEditBatch" @delete-batch="handleDeleteBatch" />
            </div>
        </div>
        <div id="tab-lessons" v-if="tab === 'lessons'">
            <div class="py-6">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="handleShowFormLesson"
                        class="justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 bg-primary text-primary-foreground shadow hover:bg-primary/90 h-9 px-4 py-2 flex items-center space-x-2">
                        <Icon icon="lucide:plus" width="24" height="24" class="w-4 h-4" />
                ...
                <div class="space-y-8 mt-8">
                    <CourseWithLessons v-for="course in coursesWithLessons" :key="course.id" :course="course"
                        @add-lesson="handleAddLesson" @edit-lesson="handleEditLesson"
